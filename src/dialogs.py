...
class Dialog:
    ...
    def __init__(self, stage, dialog_num):
        ...
        self.texts = Document(
            f'../assets/dialogs/{stage}_{dialog_num}.docx').paragraphs
        self.textbg = pygame.image.load('../assets/icons/text.png').convert()
        self.textbg.set_colorkey((0, 0, 0))
        self.button = pygame.image.load('../assets/icons/option.png').convert()
        self.talkers = self.texts[0].text.split(' ')
        self.characters = dict()
        self.font = pygame.font.Font('../assets/icons/CaslonAntique.ttf', 30)
        self.textbg.blit(
            self.font.render('press mouse to next', True, 'White'),
            (1000, 220))
        for talker in self.talkers:
            self.characters[talker] = pygame.image.load(
                f'../assets/character/{talker}.png').convert()
            self.characters[talker] = pygame.transform.rotozoom(
                self.characters[talker], 0,
                500 / self.characters[talker].get_width())
            self.characters[talker].set_colorkey(
                self.characters[talker].get_at((0, 0)))
    ...
    def gradual_typing(self, content, x, y, iskey):  # txt,字体内容。xy坐标，字体大小（默认30）
        ...
        for char in txt:
            ...
            if self.font.render(
                    rendering + char, True,
                    color).get_width() < self.textbg.get_width() - 50:
                rendering += char
            else:
                row_now += 1
                rendering = '-' + char if rendering[-1].isalpha(
                ) and char.isalpha() else char
            ...
            # name of speaker
            self.window.blit(
                (self.font.render(name.split("_")[0], True, 'Brown')),
                (textbox_rect.x + 60, textbox_rect.y + 10))
            for i in range(len(texts)):
                self.window.blit(
                    texts[i],
                    (x, y + i * self.font.get_height()))  # textbox_rect
    ...
    def options(self, opts):
        ...
        button1.blit(
            opt1,
            opt1.get_rect(center=(button1.get_width() / 2,
                                  button1.get_height() / 2)))
        button2.blit(
            opt2,
            opt2.get_rect(center=(button1.get_width() / 2,
                                  button1.get_height() / 2)))
        not_select = True
