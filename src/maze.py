...
font0 = pygame.font.Font('../assets/icons/CaslonAntique.ttf', 32)
hint = [
    'direction key: move', 'space: interaction', 'tab: open bag',
    'left shift: relive'
]
hint_sur = pygame.surface.Surface((300, 130))
for i, h in enumerate(hint):
    hint_sur.blit(font0.render(h, True, 'White'), (0, i * 32))
...
class DrawMaze(object):
    ...
    def __init__(self, stage):
        ...
        self.back = pygame.image.load(
            f"../assets/background/stage{stage}.jpg").convert()
        self.road = pygame.image.load(
            f"../assets/background/block{stage}.png").convert()
        self.monster = pygame.image.load(f"../assets/monster/{stage}.png")
        self.monster.set_colorkey(self.monster.get_at((0, 0)))
        self.chest = pygame.image.load(
            f"../assets/background/chest.png").convert()
        self.chest.set_colorkey(self.chest.get_at((0, 0)))
        self.next_arrow = pygame.image.load(f"../assets/icons/next_level.png")
        self.chest.set_colorkey(self.chest.get_at((0, 0)))
        self.talk = pygame.image.load(f'../assets/icons/talk.png').convert()
    ...
    def draw_block(self):
        ...
        for i in range(len(monster_list[0])):
            rect = (monster_list[1][i] * BLOCK + BORDER,
                    monster_list[0][i] * BLOCK + BORDER, BLOCK, BLOCK)
            self.screen.blit(self.monster, rect)
        for i in range(len(chest_list[0])):
            rect = (chest_list[1][i] * BLOCK + BORDER,
                    chest_list[0][i] * BLOCK + BORDER, BLOCK, BLOCK)
            self.screen.blit(self.chest, rect)
        rect = ((talk[1] + 0.5) * BLOCK + BORDER,
                (talk[0] + 0.5) * BLOCK + BORDER, BLOCK, BLOCK)
        self.screen.blit(self.talk, rect)
        rect = (COL * BLOCK + BORDER + 16, (ROW - 1) * BLOCK + BORDER + 16, 32,
                32)
...
class Blocks(pygame.sprite.Sprite):
    ...
    def __init__(self, num, pos):
        ...
        self.image = pygame.image.load(
            f"../assets/background/block{num}.png").convert()
        self.rect = (pos[1] * self.image.get_width() + BORDER,
                     pos[0] * self.image.get_height() + BORDER,
...
class MakeMaze(object):
    ...
    def chest_left(self):
        return np.where(self.maze == 3)[0].shape[0]

    def monster_left(self):
        return np.where(self.maze == 2)[0].shape[0]
    ...
    def random_map(self, h, w):
        ...
        for i in range(h):
            for j in range(w):
                ...
                if self.maze[i, j] == 1 and [i, j] != [0, 0] and [i, j] != [
                    ...
                    if nv <= len(neigh_now) - 2:  # grow more block
                        ...
                        self.maze[i, j] = 0
                        dj_res = self.dj()
                        if len(dj_res[0]) != 0 and self.neigh_visited(
                                self.neigh([i, j])) < 2:
                            self.maze[i, j] = 1
                else:
                    if nv < 2:
    ...
    def grow(self, cell_now):
        ...
        while neigh_now:
            ...
            if self.maze[cell_now] == 0:
                self.maze[cell_now] = 1
                break
        return cell_now

    def dj(self):
        weight = 10000 * (self.maze)
        stack = [(0, 0)]
        weight[0, 0] = 1
        while stack:
            cell_now = stack.pop()
    ...
    def neigh_visited(self, ns):
        return sum(self.maze[n] for n in ns)

    # @staticmethod
    def validpos(self, block_now, speed):
        # 判断坐标的有效性，如果超出数组边界或是不满足值为1的条件，说明该点无效返回False，否则返回True
        block_next = [
            int(block_now[0] + speed[0] / BLOCK),
            int(block_now[1] + speed[1] / BLOCK)
        ]  # 列行
        return  block_next[1] in range(self.height) and block_next[0] in range(self.width) \
    ...
    def set_stuff(self):
        ...
        self.maze[-1, -1] = 4
        dead_end = [
            tuple(p) for p in origin
            if (self.neigh_visited(self.neigh(p)) == 1 and (
                p[0] * p[1] != 0) and self.maze[p[0]][p[1]] != 4)
        ]
