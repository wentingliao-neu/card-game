...
class card:
    ...
    def __init__(self, code):  # 最多四位的字符串，用数字编码功能
        ...
        self.usages = [int(i) for i in code[1:]]
        self.name = Names[code]
        self.imagePath = f'../assets/cards/{code}.png'
        self.picture = pygame.image.load(f'../assets/cards/{code}.png')
        self.user = None
        self.object = None
        self.intro = (self.name + ': ') + ' '.join(usage[u]
    ...
    def work(self, player, enemy):
        ...
        for u in self.usages:
            if u == 1:
                ...
                self.user.current_HP = min(self.user.total_HP,
                                           self.user.current_HP + 8)
    ...
    def buffon(self, ele, u):
        ...
        if self.user.current_HP > 0 and self.object.current_HP > 0:
            ...
            match u:
                case 5:
                    self.object.buffs.append([ele, 3, buff_turns
                                              ])  # 元素，buff类型，buff持续回合
            ...
            match elements[ele]:
                ...
                case 'metal':  # 因为1和6都是金，所以给自己加buff的时候是6，破别人buff的时候是1
    ...
    def buff_cont(self):
        ...
        HP_buff = 0
        for buff in self.user.buffs:
            atk_buff = atk_buff + 1 if Buffs[
                buff[1]] == 'atk_up' else atk_buff - 1 if Buffs[
                    buff[1]] == 'atk_down' else atk_buff
            defe_buff = defe_buff + 1 if Buffs[
                buff[1]] == 'defe_up' else defe_buff - 1 if Buffs[
                    buff[1]] == 'defe_down' else defe_buff
            HP_buff = HP_buff + 1 if (
                Buffs[buff[1]] == 'recover') else HP_buff - 1 if (
                    Buffs[buff[1]] == 'burnt'
                    or Buffs[buff[1]] == 'bleeded') else HP_buff
            if buff[1] == 8:
                p = random.random()
                if p <= 0.3:
        ...
        self.user.current_HP = min(self.user.current_HP + HP_buff * 5,
                                   self.user.total_HP)
