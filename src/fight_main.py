# python3
from fight_scene import FightingScene, EndTurnButton, FightStaus
...
def fight(player, num, monster_type):
    ...
    for i in range(5):
        items.append(
            pygame.sprite.GroupSingle(
                Cards(fightS.Player.hand_card[i].name,
                      fightS.Player.hand_card[i].usages,
                      fightS.Player.hand_card[i].picture, 2 / 3,
                      start_x + (width + gap) * i, start_y)))
    ...
    while not ifquit:
            ...
            if event.type == pygame.QUIT:
                ...
                if event.button == 1:
                    ...
                    if not played_one_card_this_turn:
                        for its in range(5):
                            if items[its].sprite is not None and items[
                                    its].sprite.rect.collidepoint(click):
                                drag = True
                                select_card = its
                        ...
                        if etb.rect.collidepoint(click):
                            ...
                            monster_discard_item = pygame.sprite.GroupSingle(
                                Cards(enemy_card.name, enemy_card.usages,
                                      enemy_card.picture, 2 / 3, 60, 200))
                            ...
                            for i in range(5):
                                items.append(
                                    pygame.sprite.GroupSingle(
                                        Cards(
                                            fightS.Player.hand_card[i].name,
                                            fightS.Player.hand_card[i].usages,
                                            fightS.Player.hand_card[i].picture,
                                            2 / 3, start_x + (width + gap) * i,
                                            start_y)))
        ...
        if fightS.Monster.current_HP <= 0 or player.current_HP <= 0:
            player.battle_end()
            return fightS.Monster.current_HP <= 0
