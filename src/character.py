...
from settings import ROW, COL, BLOCK, BORDER
from card import card

SPEED = 8
pygame.display.init()

card_will_get = [['5123', '51', '52'], ['112', '116', '114'],
                 ['21', '225', '2135']]
...
class Character:
    ...
    def __init__(self, name, code):
        ...
        if code == 0:
            self.block = [0, 0]
            self.chara = 'player'
            file_path = '../assets/character/hero/'
            self.chi = 0
        else:
            self.block = [ROW - 1, COL - 1]
            self.chara = 'boss'
            file_path = f'../assets/character/boss{code}/'

        self.rect = pygame.Rect(self.block[1] * BLOCK + BORDER,
                                self.block[0] * BLOCK + BORDER, 64, 64)
        self.card_deck = [
            card('01'),
            card('01'),
            card('01'),
        ...
        self.hand_card = []
        self.drop_card = []
        self.attack_se = pygame.mixer.Sound('../assets/bgm/attack.mp3')
    ...
    def draw_self(self, screen, frame_num, is_moving, is_atk, dead_time):
        ...
        if is_moving:
            ...
            if time_now - dead_time < 100 * len(self.dead_pics):
                pic_now = self.dead_pics[((time_now - dead_time) // 100) %
                                         len(self.dead_pics)]
            else:
                pic_now = self.dead_pics[-1]

        else:
            pic_now = self.static_pics[(frame_num // 10) %
                                       (len(self.static_pics))]
    ...
    def move(self, p):
        ...
        if self.rect.topleft[0] != self.block[0] + BLOCK + BORDER or \
            ...
            self.rect.move_ip(self.speed[0] * SPEED / BLOCK,
                              self.speed[1] * SPEED / BLOCK)
            return True, p + 1
            ...
            match maze0.maze[n]:
                ...
                case 3:
                    self.card_deck.append(
                        card(card_will_get[maze0.num -
                                           1][3 - maze0.maze.chest_left()]))
                    self.current_HP = self.total_HP
                    maze0.maze.kill(n)
                ...
                case 4:
                    ...
                    if res:
                        ...
                        self.infected(kn[0])
                        boss_die_time = pygame.time.get_ticks()
                        maze0.maze.kill(n)
                        break
                    else:
                        self.dead = True
    ...
    def battle_end(self):
        ...
        self.card_deck.extend(self.drop_card)
        self.drop_card.clear()

        self.card_deck.extend(self.hand_card)
    ...
    def check_dire(self, maze, up, left, right, down, moving, p):
        if moving:
            ...
            if down:
                self.speed = [0, BLOCK]
            if maze.validpos(self.block,
                             self.speed):  # 合法性判断,x对应数组的列，y坐标对应数组行，注意别弄反了
